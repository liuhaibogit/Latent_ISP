...
def num_parameters(model: Type[nn.Module]) -> int:
    """
    :param model: Torch networks
    :return: Number of parameters in networks.
    """
...
def get_model(log_dir: str, load_params: bool=False) -> Tuple[Type[nn.Module],Dict]:
    """
    Returns networks used for experiment stored in ``log_dir``.
    ...
    with open(os.path.join(log_dir,'args.json')) as j:
        args = json.load(j)
    model = getattr(models, args['networks'])(args['in_features'], args['out_features'], **args)
    if load_params:
